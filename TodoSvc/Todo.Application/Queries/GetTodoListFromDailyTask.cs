...
public class GetTodoListFromDailyTask : IRequest<ResultCustomPaginate<IEnumerable<DailyTaskDto>>>
{
    public int PageNumber { get; init; } = 1;
    public int PageSize { get; init; } = 10;
}

public class GetTodoListFromDailyTaskHandler : IRequestHandler<GetTodoListFromDailyTask, ResultCustomPaginate<IEnumerable<DailyTaskDto>>>
{
    private readonly ITodoDbContext _context;
    private readonly IUser _user;
    private readonly IPerfDbContext _perfContext;
    private readonly IApplicationDbContext _tenantContext;
    public GetTodoListFromDailyTaskHandler(ITodoDbContext context, IUser user, IApplicationDbContext tenantContext, IPerfDbContext perfDbContext)
    {
        _context = context;
        _user = user;
    ...
    public async Task<ResultCustomPaginate<IEnumerable<DailyTaskDto>>> Handle(GetTodoListFromDailyTask request, CancellationToken cancellationToken)
    {
        try
        {
            // Load TodoItems
            var todoItems = await _context.TodoItems
                .Where(t => t.Assignee == Guid.Parse(_user.Id) && t.DueDate == DateTime.Today)
            ...
            return new ResultCustomPaginate<IEnumerable<DailyTaskDto>>
            {
                ...
                TotalItems = await _context.TodoItems.CountAsync(t => t.Assignee == Guid.Parse(_user.Id) && t.DueDate == DateTime.Today, cancellationToken)
            };
        }
        catch (Exception ex)
        {
